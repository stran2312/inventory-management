...
if(isset($_REQUEST['edit_device'])){
	...
	$sql = "UPDATE `device_table` SET device_type = '$device_type', manufacturer = '$manufacturer', serial_number = '$serial_number', status = '$status' WHERE auto_id = '".$_REQUEST['auto_id']."'";
	$result = $dblink->query($sql);
	if($result == true){
...
if(isset($_POST["UploadAppDoc"]) && $_FILES['userfile']['size'] > 0){
					...
					redirect("https://ec2-3-137-205-7.us-east-2.compute.amazonaws.com/manage.php?auto_id=$auto_id&execTime=$execution_time");
}
				...
				<form action="manage.php" method="POST" style="margin-bottom: 16px;">
				  ...
				  <div class="form-check">	
					<label for="status" style="margin-right: 10px">Status</label>  
				  	<input type="checkbox" checked data-toggle="toggle" name="checkbox">
				  </div>	
				...
				<form action="manage.php?auto_id=<?php echo $row->auto_id;?>" method="POST" enctype="multipart/form-data">
					...
					<div class="form-group">
						...
						<div class="fileupload" data-provides="fileupload">
							...
							<a href="manage.php?auto_id=<?php echo $row->auto_id;?>" class="btn btn-danger" data-dismiss="fileupload">Clear</a>
						</div>
